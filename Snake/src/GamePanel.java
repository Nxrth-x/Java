...
public class GamePanel extends JPanel implements ActionListener {
  ...
  int bodyParts = 6;
  Color bodyColor = new Color(0, 119, 182);
  int applesEaten;
  int appleX;
  int appleY;
  char direction = 'R';
  ...
  public void checkCollisions() {
    // Checking if the head collided with any of the body parts
    for (int i = bodyParts; i > 0; i--) {
      if ((x[0] == x[i]) && (y[0] == y[i])) {
        running = false;
      }
    ...
    // Check if the snake collided with the edges of the screen
    if (x[0] < 0) {
      running = false;
  ...
  public void gameOver(Graphics g) {
    ...
    g.drawString("Final score: " + applesEaten,
        (SCREEN_WIDTH - score_metrics.stringWidth("Final score: " + applesEaten)) / 2, SCREEN_HEIGHT / 10);
  }
